...
The issue tracker and triage label vocabulary live in `docs/agents/issue-tracker.md` and `docs/agents/triage-labels.md` — read them if you haven't already.
...
> **Autonomous run** (epic run after gate 1 — see "Pre-autorizzazione del run" in `docs/agents/workflow.md`): do not stop to ask. Answer the same three questions yourself as a self-review, adjust the breakdown, then go straight to step 5. After publishing, continue with the definition of the next feature (`/to-spec` → `/to-tickets`) until **every** feature of the epic is defined — implementation starts only after that, and it is not a question to put to the Creator.
...
Work the frontier one ticket at a time with `/implement`, clearing context between tickets. In the epic lifecycle of `docs/agents/workflow.md`, this starts only after **all** features are defined.
